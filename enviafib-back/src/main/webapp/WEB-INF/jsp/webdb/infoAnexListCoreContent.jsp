...
<un:useConstants var="InfoAnexFields" className="es.caib.enviafib.model.fields.InfoAnexFields"/>
        ...
        <c:if test="${ __entry.key < 0  && ((empty __entry.value.searchBy)? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.searchBy)) && ((empty __entry.value.groupBy )? true : !gen:contains(__theFilterForm.hiddenFields, __entry.value.groupBy ))}">
          <td>
             <c:if test="${not empty __entry.value.valueMap }">
               <c:out escapeXml="${__entry.value.escapeXml}" value="${__entry.value.valueMap[infoAnex.infoanexid]}" />
             </c:if>
        ...
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,InfoAnexFields.PETICIOID)}">
          <td>
          <c:set var="tmp">${infoAnex.peticioID}</c:set>
        ...
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,InfoAnexFields.ANEXID)}">
          <td>
            <c:if test="${not empty infoAnex.anex}">
              <a target="_blank" href="<c:url value="${efi:fileUrl(infoAnex.anex)}"/>">${infoAnex.anex.nom}</a>
            </c:if>
           </td>
        </c:if>
