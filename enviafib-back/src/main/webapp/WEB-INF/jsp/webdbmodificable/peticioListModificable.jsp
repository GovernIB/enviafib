...
function downloadSelectedFiles(){
    ...
    console.log("Enviando petici�n para descargar los siguientes ficheros:", selectedItems);
...
function downloadSelectedFilesOld(){
    ...
    // Enviar los datos en el cuerpo de la petici�n
    fetch(url, {
        method: 'POST',
        headers: {
...
<div id="page-header" class="float-right">
	<div id="botoneraContainer">
		...
		</a> <input id="btn-clean" class="btn btn-filtre float-right"
			type="button" onclick="clear_form_elements(this.form)"
			value="<fmt:message key="genapp.form.clean"/>" /> <input
			id="btn-submit" class="btn btn-filtre btn-secondary float-right"
			type="submit" value="<fmt:message key="genapp.form.search"/>" />
	</div>
</div>
...
<div id="page-body" class="form-inline">
	...
	<c:if
		test="${gen:contains(__theFilterForm.filterByFields ,PeticioFields.NOM)}">
		<%-- FILTRE STRING - NOM --%>
	...
	<c:if
		test="${gen:contains(__theFilterForm.filterByFields ,PeticioFields.DATACREACIO)}">
		<%-- FILTRE DATE-TIME --%>
	...
	<c:if
		test="${gen:contains(__theFilterForm.filterByFields, PeticioFields.ESTAT)}">
		<div class="filtre-div input-group">
			<%-- FILTRE NUMERO SELECT MULTIPLE --%>

			<div class="input-group-prepend" id="div_label_estat">
				<span class="add-on" style="margin: auto; color: #6C757D;"><fmt:message
						key="peticio.estat" />:</span>
			</div>
			...
			<div class="input-group-prepend"
				...
				<form:select id="peticio_estat_select" path="estatSelect"
					...
					multiple="true" style="width:100%;" tabindex="-1"
					aria-hidden="true">
...
.select2-selection__choice {
	...
	<c:if
		test="${gen:contains(__theFilterForm.filterByFields ,PeticioFields.DATAFINAL)}">
		<%-- FILTRE DATE-TIME --%>
		<div class="filtre-div input-group">
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<button type="button" class="close" aria-label="Close"
					onclick="$('#fluxInfoModal').modal('hide')">
					<span aria-hidden="true">&times;</span>
				</button>
			</div>
			<div class="modal-body" style="text-align: center;">
...
$(document).ready(function(){
    $('.estatInfo').each(function(){
        $(this).closest('td').addClass('tdEstat');
    });
});
